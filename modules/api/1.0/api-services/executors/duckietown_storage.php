...
function execute(&$service, &$actionName, &$arguments) {
    ...
    switch ($actionName) {
        case 'has_permission':
            ...
            $res = Duckietown::checkStorageSpacePermissions(
        ...
        case 'grant_permission':
            ...
            if ($res['data']['type'] == 'uid') {
                $res = Duckietown::grantStorageSpacePermissionToUser(
                    $res['data']['id'], $arguments['bucket'], $arguments['object'],
                    $arguments['action']
                );
            } else {
                $res = Duckietown::grantStorageSpacePermissionToGroup(
                    $res['data']['id'], $arguments['bucket'], $arguments['object'],
                    $arguments['action']
                );
            }
        ...
        case 'revoke_permission':
            ...
            // we have either uid or gid
            if ($res['data']['type'] == 'uid') {
                $res = Duckietown::revokeStorageSpacePermissionToUser(
                    $res['data']['id'], $arguments['bucket'], $arguments['object'],
                    $arguments['action']
                );
            } else {
                $res = Duckietown::revokeStorageSpacePermissionToGroup(
                    $res['data']['id'], $arguments['bucket'], $arguments['object'],
                    $arguments['action']
                );
            }
